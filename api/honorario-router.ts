...
export const honorarioRouter = createRouter({
  ...
  reporteCobranza: publicQuery.query(async () => {
    const db = getDb();
    const todos = await db.select().from(honorarios);
  ...
  generarCartaCobranza: publicQuery
  ...
  resumenMensual: publicQuery.query(async () => {
    const db = getDb();
    const todos = await db.select().from(honorarios);
